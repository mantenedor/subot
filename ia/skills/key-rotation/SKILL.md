---
name: key-rotation
description: Rotaciona o par de chaves SSH do bastião (e orienta a atualização de authorized_keys nos hosts gerenciados) usando bastiao/scripts/rotate-ssh-keys.sh. Use quando o pedido for sobre rotação, renovação ou comprometimento de chaves SSH.
---

# key-rotation

1. Rode `bash bastiao/scripts/rotate-ssh-keys.sh` — gera um novo par em `./bastiao/secrets/ssh/` com sufixo de
...
3. Só remova a chave antiga de `./bastiao/secrets/ssh/` depois de validar que todos os hosts do grupo
