...
if [ ! -f "$KEY" ]; then
    ...
    ssh-keygen -t ed25519 -f "$KEY" -N "$PASSPHRASE" -C "subot-bastion"
    chmod 600 "$KEY"
    chmod 644 "${KEY}.pub"
    echo ""
    echo "    #################################################################"
    ...
    echo "    #   ${PASSPHRASE}"
    echo "    #"
    echo "    # Pra subir a stack, exporte antes do 'docker compose up':"
    echo "    #   export SUBOT_SSH_KEY_PASSPHRASE='${PASSPHRASE}'"
    echo "    #################################################################"
    echo ""
