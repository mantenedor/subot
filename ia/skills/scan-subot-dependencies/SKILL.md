...
description: Varre as dependências do próprio subot (pacotes Python de bastiao/packages/subot_core e bastiao/packages/subot_orchestrator, e imagens Docker do bastiao/docker-compose.yml/containers/) em busca de CVEs conhecidas, e registra achados em data/security-findings/subot.jsonl. Use quando o pedido for para verificar vulnerabilidades no código/imagens do próprio subot, não em hosts gerenciados.
---
...
1. **Dependências Python:** para cada `requirements*.txt`/`pyproject.toml` sob
   `bastiao/packages/subot_core/` e `bastiao/packages/subot_orchestrator/`, rode `pip-audit -r <arquivo>` se
...
2. **Imagens Docker:** extraia as tags de imagem referenciadas em `bastiao/docker-compose.yml` e em
   qualquer `bastiao/containers/*/Dockerfile` (linhas `FROM`). Se `trivy` estiver disponível
