---
name: backup-restore
description: Faz backup ou restauração dos volumes persistidos do subot (config, dados de auditoria, banco do Guacamole, chaves) usando bastiao/scripts/backup.sh e bastiao/scripts/restore.sh. Use quando o pedido envolver backup, restore, ou recuperação de desastre do próprio subot.
---
...
- Restore: rode `bash bastiao/scripts/restore.sh <arquivo.tar.gz>`. **Ação destrutiva** — sempre confirme
