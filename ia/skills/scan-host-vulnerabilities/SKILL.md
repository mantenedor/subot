...
description: Varre hosts gerenciados (domain/) em busca de atualizações de segurança/CVEs conhecidas nos pacotes instalados, e registra achados em data/security-findings/hosts.jsonl. Use quando o pedido for para verificar vulnerabilidades, CVEs ou pendências de patch nos hosts gerenciados.
---
...
1. Liste os hosts alvo via `domain/` (ou `mcp__inventory_connector__list_hosts` quando
...
2. Para cada host, detecte a família de SO antes de escolher o comando (`cat /etc/os-release`).
   ...
   - Hosts `protected`/`prod` (tag em `host.yaml`): mesmo sendo comandos `safe`, redobre a atenção
...
4. Registre uma linha JSON por host varrido (append, nunca sobrescreva o arquivo), mesmo sem
   ...
   ```json
   {"ts": "<ISO8601 UTC>", "host": "<nome-do-diretório-em-domain/>", "os": "<id do os-release>", "findings": [{"id": "<CVE-ou-advisory>", "package": "<nome>", "severity": "<critical|high|medium|low>"}], "recommendation": "<texto objetivo>"}
   ```
